<template>
    <v-app>
        <v-app-bar app>
            <v-tabs v-model="tabIndex" background-color="blue accent-3" centered dark icons-and-text>
                <v-tabs-slider></v-tabs-slider>
                <v-tab :href="`/diseaseRobot`">
                    Principal
                    <v-icon>mdi-home</v-icon>
                </v-tab>
                <v-tab :href="`/area_medico/${user}`">
                    Área Médico
                ...
                <v-tab :href="`/pacientes/${medico}/${user}`">
                    Pacientes
        ...
        <v-main>
            <v-container>
                <h3 class="mt-5 teal--text d-flex justify-center">Configura Tu Calendario Médico</h3>
                <v-expansion-panels>
                    <h5 class="mt-5 teal--text text-accent-4 d-flex justify-center">Instrucciones para configurar el
                        calendario (<span class="red--text darken-3">Lea cuidadosamente antes de usar.</span>)</h5>
                    ...
                    <v-col>
                        <v-sheet height="600">
                            <v-calendar ref="calendar" locale="es" :weekdays="[1, 2, 3, 4, 5, 6, 0]" first-interval="7"
                                interval-count="15" v-model="value" color="primary" type="week" :events="events"
                                :event-color="getEventColor" @click:event="showEvent" @mousedown:event="startDrag"
...
export default {
    data: () => ({
        ...
        instruc:
        {
            text: ['1. Es muy fácil de usar. Sólo debe presionar un click dentro del calendario y tendrá su botón de cita creado automáticamente.', '2. En Minutos de Cita tendrá solo dos tiempos de cita: De 30 minutos y De 60 minutos. El botón de cita automáticamente se ajustará a su selección.', '3. En Cantidad de Citas puede seleccionar la cantidad de botones de cita que se crearán entre 1 Día ó 1 año, al configurar una sola semana automáticamente se crearán botones consecutivos para la selección que usted escoja. Tenga cuidado porque una vez creados los botones de cita no podrán eliminarse en lote es decir si desea eliminarlos debe ser uno por uno. No se puede eliminar un grupo de botones de cita. Por último cuándo se crean grupos de botones de cita debe esperar que el sistema cargue. Considere que si selecciona 6 Meses o 1 Año el tiempo de retraso es mayor que el de 1 Día ó 1 Mes.'],
