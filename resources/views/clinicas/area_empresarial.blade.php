...
<head>
    ...
    <link href="{{asset('startbootstrap-freelancer-gh-pages\startbootstrap-freelancer-gh-pages\css\enterprise.css')}}" rel="stylesheet" />
</head>
<style type="text/css">
            .my-custom-scrollbar {
        position: relative;
...
<header class="masthead bg-light  text-light text-center encabezado" >
        <h2 class="fuente_e">&Aacute;rea Empresarial</h2>
</header>
<div class="container-fluid mt-4">
    <div class="row">
        <div class="col-md-4 my-2">
                <div class="card p-3" >
                    <h4 class="text-primary">Perfil Empresarial</h4>
                    <div class="row">
                        @foreach($facturas_clinicas as $f)
                        <div class="col-md-6">
                            <div class="text-center">
                               <a class="btn btn-success btn-md my-3 crear_perfil"   href="{{route('perfil_empresarial_form_fill')}}"><i class="bi bi-shield-plus"></i> Crear {{ucfirst($f->tipo_negocio)}}</a> 
                            </div>

                        <div class="text-center">
                                <a id="botonc" class="btn btn-primary btn-md my-3 "   href="{{route('perfil_empresarial',$clinica ?? '')}}"><i class="bi bi-eye"></i> Ver {{ucfirst($f->tipo_negocio)}}</a> 
                            </div>
                        </div>
                        <div class="col-md-6">
                                <div class="text-center ">
                                    <a id="boton" class="btn btn-secondary btn-md my-3 " href="{{route('clinicas_editar',$clinica ?? '')}}" ><i class="bi bi-pencil"></i> Editar {{ucfirst($f->tipo_negocio)}}</a>
                                </div>
                                <div class="text-center">
                                    <form class="my-3 eliminar_clinicas"  method="POST" action="{{route('delete_perfil_empresarial',$clinica ?? '')}}"> 
                                     @csrf
                                     @method('delete')
                                     <button id="botona" type="submit" class="btn btn-danger btn-md " value=""><i class="bi bi-shield-x"></i> Borrar {{ucfirst($f->tipo_negocio)}}</button>
                                    </form>
                                </div>
                        </div>
                    </div>
                    @endforeach
                        @if(isset($clinica) === false)
                ...
                <h4 class="text-primary">Pagos de Plan</h4>
			     <div class="table-responsive ">
    			   ...
    			   <table class=" table table-dark table-striped">			
                            ...
                            <tr>
                            @foreach($facturas_clinicas as $f)
                            <td><i>{{ucfirst($f->tipo_negocio)}}, {{ucfirst($f->nombre_negocio)}}</i></td>
                            @endforeach
                            ...
                            <td><p href="" class="btn btn-sm btn-primary" disabled>Cambiar Plan</p></td>
                            </tr>
    			    </table>
			     </div>
            </div>
        ...
        <div class="row">
            <div class="col-md-4 my-2">
                    ...
                    <h4 class="text-primary">Datos de Factura</h4>
                         @foreach($facturas_clinicas as $f)
                        <i>{{ucfirst($f->tipo_negocio)}}, {{ucfirst($f->nombre_negocio)}}. Nº: {{$f->rif_negocio}}</i>
                        <b class="fuente_e">Representante</b>
                        <i>{{ucfirst($f->nombre_representante)}}, {{ucfirst($f->apellido_representante)}}. TLF: {{$f->celular}} <br>
                         Nº Identificaci&oacute;n: {{$f->ci_rif}}.</i>
                        <b class="fuente_e">Direcci&oacute;n</b>
                        <i>{{$f->estado}}, {{ucfirst($f->ciudad)}},</i>
            ...
            <div class="card p-3" >
                <h4 class="text-primary">Pedidos</h4>
			     <div class="table-responsive-sm table-wrapper-scroll-y my-custom-scrollbar">
                    ...
                    @if($p->plan === 'cisne')
                                    <div id="paypal-button-container-P-7LE80325R1158114JMQ2AQMY"></div>
                                      ...
                                      paypal.Buttons({
                                          ...
                                          createSubscription: function(data, actions) {
                                            return actions.subscription.create({
                                              ...
                                              plan_id: 'P-7LE80325R1158114JMQ2AQMY'
                                            });
                                          },
                                          onApprove: function(data, actions) {
                                            alert(data.subscriptionID); // You can add optional success message for the subscriber here
                                          }
                                      }).render('#paypal-button-container-P-7LE80325R1158114JMQ2AQMY'); // Renders the PayPal button
                                    </script>
                                @elseif($p->plan === 'aioria')
                                    <div id="paypal-button-container-P-4MV79914HB584113FMQ2AR4I"></div>
                                    <script src="https://www.paypal.com/sdk/js?client-id=AcjKr1dIRIlqcsimhDghBhccQXif2wgwS5V4todPbBerLYSeQIP7eFqjw-EZ1eIs5QPFgX84-sHBQCJT&vault=true&intent=subscription" data-sdk-integration-source="button-factory"></script>
                                    <script>
                                      paypal.Buttons({
                                          style: {
                                          ...
                                          createSubscription: function(data, actions) {
                                            return actions.subscription.create({
                                              /* Creates the subscription */
                                              plan_id: 'P-4MV79914HB584113FMQ2AR4I'
                                            });
                                          },
                                          onApprove: function(data, actions) {
                                            alert(data.subscriptionID); // You can add optional success message for the subscriber here
                                          }
                                      }).render('#paypal-button-container-P-4MV79914HB584113FMQ2AR4I'); // Renders the PayPal button
                                    </script>
                                @elseif($p->plan === 'dragon')
                                    <div id="paypal-button-container-P-3SN931229W762125DMQ2AUPI"></div>
                                    <script src="https://www.paypal.com/sdk/js?client-id=AcjKr1dIRIlqcsimhDghBhccQXif2wgwS5V4todPbBerLYSeQIP7eFqjw-EZ1eIs5QPFgX84-sHBQCJT&vault=true&intent=subscription" data-sdk-integration-source="button-factory"></script>
                                    <script>
                                      paypal.Buttons({
                                          style: {
                                          ...
                                          createSubscription: function(data, actions) {
                                            return actions.subscription.create({
                                              /* Creates the subscription */
                                              plan_id: 'P-3SN931229W762125DMQ2AUPI'
                                            });
                                          },
                                          onApprove: function(data, actions) {
                                            alert(data.subscriptionID); // You can add optional success message for the subscriber here
                                          }
                                      }).render('#paypal-button-container-P-3SN931229W762125DMQ2AUPI'); // Renders the PayPal button
                                    </script>
                                @endif
                                </div>
                        @endforeach   
    			   <table class="table table-dark table-striped table-sm " >			
                            ...
                            <td>{{$p->total}}</td>
                            <td><a target="_blank" href="{{route('pagando_perfil_empresarial_cisne',[$user,$p->id])}}" class="btn btn-sm {{$p->estado === 'No Pagado' ?'btn-danger': 'btn-success'}}">{{$p->estado}}</a></td>
        				    </tr>
        				   @endforeach
    			    </table>
			     </div>
     ...
     <div class="row">
        <div class="col-md-4 my-2">
            <div class="card p-3">
                 <h6 class="text-primary">Reportar Pagos</h6>
                    <div class="text-center">
                       <a class="btn btn-primary my-1 crear_perfil"   href="{{route('reporte_pago_empresarial',$user)}}"><i class="bi bi-credit-card"></i> Reporte Pago</a> 
                    </div>
                    <h6 class="text-primary">Saldo de {{ucfirst($f->tipo_negocio)}} {{ucfirst($f->nombre_negocio)}}</h6>
                    <div class="row">
                        <div class="col-md-3">
                           <b>USD</b>
                        </div>
        ...
        <script src="{{asset('bootstrap-4.0.0\bootstrap-4.0.0\assets\js\vendor\holder.min.js')}}"></script>
        		...
        		<!--Monkey D Luffy-->
        <script type="text/javascript" src="{{asset('startbootstrap-freelancer-gh-pages\startbootstrap-freelancer-gh-pages\js\eliminar_clinicas.js')}}"></script>
        @if (session('status'))
        <script src="{{asset('startbootstrap-freelancer-gh-pages\startbootstrap-freelancer-gh-pages\js\crear_perfil_empresarial.js')}}"></script>
        @endif
        @if (session('statusb'))
