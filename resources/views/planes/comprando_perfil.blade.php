...
<body>
	...
	<b>Facturado a:</b><br>
	<i>{{ucfirst($f->nombre)}} {{ ucfirst($f->apellido)}}</i>,<br>
	<i>{{$f->estado}}, {{$f->ciudad}}, {{$f->direccion}}, {{$f->codigo_postal}}</i><br>
	<i>Identificación:(Para {{ucfirst($f->estado)}}): {{$f->ci_rif}}</i><br>
	<i>Teléfono:</i> <i>{{$f->celular}}.</i><br>
	
	@endforeach
{{--	<h2>Pague <span class="colorb">${{$pedido->total}}</span> a la siguiente cuenta:</h2>
	...
	<div>
	    <table class="table-head table" >
	      <tr >
	        ...
	        <tr>
	            <td class="tr-head">Plan {{$pedido->plan}}. Crear, Editar, Eliminar M&eacute;dico. Aparecer en listas de b&uacute;squeda * </td>
	            <td class="tr-head">$.{{$pedido->total}}</td>
	        </tr>
	        <tr>
	            <td class="tr-head back" align="right"><b>Sub Total</b></td>
	            <td class="tr-head back">$.{{$pedido->sub_total}}</td>
	        </tr>
	        <tr>
	            <td class="tr-head back" align="right"><b>16.00% Iva</b></td>
	            <td class="tr-head back">$.{{$pedido->iva}}</td>
	        </tr>
	        <tr>
	            <td class="tr-head back" align="right"><b>Total</b></td>
	            <td class="tr-head back">$.{{$pedido->total}}</td>
	        </tr>
	    </table>
	...
	<div>
	    <table class="table-head table">
	        <tr>
	            <th align="left" class="tr-head back">Fecha Reporte</th>
